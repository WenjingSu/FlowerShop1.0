...
function handlePaginationClick(new_page_index, pagination_container) {
   	 $("#Form").attr("action", "<%=path%>/OrderBackServlet?method=pageOrderList&currentPage=" + (new_page_index+1));
   	 $("#Form").submit();
     return false;
	}
	$(function(){
...
<body>
	<div class="wrap">
		...
		<div class="operate">
			<form action="${pageContext.request.contextPath}/OrderBackServlet?method=pageOrderList" method="post" id="Form">
				...
				<input type="text"
					...
					<option value="4">4.交易完成（已收货）</option>
					<option value="5">4.交易完成（已评价）</option>
			...
			<c:forEach items="${list.dataList}" var="orderlist">
				<tr>
					...
					<td class="center"><c:if test="${orderlist.orderstate==1}">
							...
							<span><c:out value="交易完成（待评价）" /></span>
						</c:if><c:if test="${orderlist.orderstate==5}">
							<span><c:out value="订单完成" /></span>
						</c:if></td>
					<td class="center"><a
						href="${pageContext.request.contextPath}/OrderBackServlet?method=order_detail&oid=${orderlist.order_id}"
