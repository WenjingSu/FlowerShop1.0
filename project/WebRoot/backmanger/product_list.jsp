...
<script type="text/javascript" src="backmanger/js/public.js"></script>
   ...
   <script type="text/javascript">
       ...
       window.location.href = "<%=path%>/GoodsServlet?method=Batchdel&deleid=" + deleid;
 	 }
  ...
  <body>
     <div class="wrap">
       ...
       <span class="modular fr"><a href="${pageContext.request.contextPath}/GoodsServlet?method=add_product" class="pt-link-btn">+添加商品</a></span>
       </div>
       ...
       <td class="center"><input type="checkbox" name="one" value="${list.g_id}" id="one"/>&nbsp;<span class="middle"><i>${id.index+1}</i></span></td>
       
       
       <td class="center pic-area"><img src="img/${list.g_imgurl}" class="thumbnail"/></td>
       <td class="td-name">
       <span class="ellipsis td-name block">${list.g_name}</span>
       </td> 
       ...
       <td class="center">
       <span>
       <em><c:if test="${list.g_state==1}"><a href="${pageContext.request.contextPath}/GoodsServlet?method=state&id=${list.g_id}&id1=2" title="点击下架"><input type="button" value="已上架"></a></c:if>
       <c:if test="${list.g_state!=1}"><a href="${pageContext.request.contextPath}/GoodsServlet?method=state&id=${list.g_id}&id1=1" title="点击上架"><input type="button" value="未上架"></a></c:if></em>
       </span>
       </td>
       ...
       <a href="${pageContext.request.contextPath}/GoodsServlet?method=update_product_skip&g_id=${list.g_id}" title="编辑基本信息"><img src="images/icon_edit.gif"/></a>
       <a href="${pageContext.request.contextPath}/GoodsServlet?method=edit_g_flowerType_skip&g_id=${list.g_id}" title="编辑花材信息"><img src="images/icon_edit.gif"/></a>
       <a href="${pageContext.request.contextPath}/GoodsServlet?method=edit_g_flowerUse_skip&g_id=${list.g_id}" title="编辑用途信息"><img src="images/icon_edit.gif"/></a>
       <a href="${pageContext.request.contextPath}/GoodsServlet?method=edit_images_skip&g_id=${list.g_id}" title="编辑图片信息"><img src="images/icon_edit.gif"/></a>
       <a href="${pageContext.request.contextPath}/GoodsServlet?method=del_and_recycle&id=${list.g_id}&id1=0" title="删除" class="del"><img src="images/icon_drop.gif"/></a>
       </td>
       </tr>
       </c:forEach>
  </table>
